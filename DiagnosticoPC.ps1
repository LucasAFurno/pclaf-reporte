...
.lights{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-top:10px}
...
.section-sub{padding:0 20px 14px;color:var(--text2);font-size:13px;line-height:1.6}
.piece-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;padding:0 20px 20px}
.piece{
...
.stat{border:1px solid rgba(255,255,255,.07);border-radius:16px;padding:10px;background:rgba(255,255,255,.018)}
.stat-k{font-family:var(--mono);font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.1em;margin-bottom:7px}
.stat-v{font-size:15px;font-weight:800;line-height:1.35;color:#fff}
.stat-v small{display:block;color:var(--text2);font-size:11px;font-weight:500;margin-top:4px}
.cluster-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;padding:0 20px 20px}
...
.cluster-body{padding:0 16px 16px;display:grid;gap:10px}
...
.resume-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;padding:0 20px 20px}
.resume-card{border:1px solid rgba(255,255,255,.07);border-radius:18px;padding:12px;background:rgba(255,255,255,.02)}
.resume-main{grid-column:span 2}
...
.resume-v{font-size:20px;font-weight:900;color:#fff;line-height:1.15}
.resume-copy{margin-top:6px;color:var(--text2);font-size:12px;line-height:1.55}
...
function Get-ThermalOverviewHtml {
    ...
    foreach ($row in $orderedRows) {
        ...
        $tempPct = 6
        $loadVal = "N/D"
        $stateText = if ([string]::IsNullOrWhiteSpace([string]$row.Celsius) -or [string]$row.Celsius -eq "N/D") {
            "Sin sensor"
        } elseif ($row.Estado) {
            [string]$row.Estado
        } else {
...
$BrandMark = "<img src='data:image/png;base64,$LogoB64' alt='PCLAF'>"
